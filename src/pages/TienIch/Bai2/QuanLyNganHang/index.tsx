...
const QuanLyNganHang: React.FC = () => {
	...
	const themMon = (values: { maMon: string; tenMon: string; soTinChi: number; khoiId: string }) => {
		...
		setMon((prev) => [
			...prev,
			{ id: taoId(), maMon: ma, tenMon: ten, soTinChi: values.soTinChi, khoiId: values.khoiId },
		]);
		formMon.resetFields();
	};
	...
	const taoDeThi = (values: { monHocId: string; khoiId: string } & YeuCauDe) => {
		...
		for (const muc of mucDoOptions) {
			const danhSach = cauHoi.filter(
				(item) => item.monHocId === monHocId && item.khoiId === khoiId && item.mucDo === muc,
			);
	...
	const tongCauHoiTheoMon = useMemo(() => {
		return mon.map((monHoc) => ({
			monHocId: monHoc.id,
			tong: cauHoi.filter((cau) => cau.monHocId === monHoc.id).length,
		}));
	}, [mon, cauHoi]);
					...
					<Card title='4. Quản lý đề thi'>
						<Form form={formDeThi} layout='vertical' onFinish={taoDeThi}>
							...
							<Space>
								...
								{deThiDangSua && <Button onClick={huySuaDeThi}>Hủy chỉnh sửa</Button>}
							</Space>
						</Form>
						<List
							style={{ marginTop: 16 }}
